...
void __init e820__memblock_setup(void)
{
	 ...
	 */
	memblock_allow_resize();  // A - 允许 memblock 动态调整大小，以适应更多的 e820 条目

	for (i = 0; i < e820_table->nr_entries; i++) {  // A - 遍历所有 e820 表项
		struct e820_entry *entry = &e820_table->entries[i];

		end = entry->addr + entry->size;   // B - 计算内存区域的结束地址
		if (end != (resource_size_t)end)   // C - 如果结束地址溢出，则跳过该区域
			continue;

		if (entry->type == E820_TYPE_SOFT_RESERVED)  // D - 如果是软保留内存区域
			memblock_reserve(entry->addr, entry->size);  // D - 将该区域标记为保留，不参与分配

		if (entry->type != E820_TYPE_RAM && entry->type != E820_TYPE_RESERVED_KERN)  // E - 如果不是可用内存或内核保留内存
			continue;

		memblock_add(entry->addr, entry->size);  // F - 将可用内存添加到 memblock 中
	}

	/* Throw away partial pages: */
	memblock_trim_memory(PAGE_SIZE);  // G - 丢弃部分页（不完整页）

	memblock_dump_all();  // H - 打印所有 memblock 内存区域信息，供调试使用
}
